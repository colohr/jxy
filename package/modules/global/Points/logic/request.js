window.fxy.exports('Points',(Points,fxy)=>{
	...
	const Request = {
		...
		headers(endpoint,content_type){
			let type = content_type === 'json' ? content_type:fxy.file.type(endpoint)
			let value = null
			switch(type){
				case 'js':
				case 'es6':
	...
	function get_json(data){
		if(fxy.is.text(data) && fxy.is.json(data)) return data
		if(fxy.is.object(data)===false) return null
	...
	function request_get(){
		...
		//shared actions
		function get_data(endpoint){ return Points.request.get(endpoint,'json') }
		
		function get_module(file){
			return fxy.port.eval(file)
		}
	}
	...
	function request_headers(request,headers){
		for(let name in headers) {
			...
			if(!fxy.is.nothing(value)) request.setRequestHeader(name,value)
		}
		return request
	}
	...
	function request_post(){
		...
		//shared actions
		function post_data(endpoint){
			return Points.request.post(endpoint,'json')
		}
	}
